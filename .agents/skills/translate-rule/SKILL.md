...
你的任务是将我指定的规则文档翻译为中文。我会指定一个位于 `translation/input/` 文件夹中的YAML文件（如果在该文件夹中找不到我指定的文件，则报告错误并停止后续流程）。该YAML文件以结构化的形式编写了《矩阵潜袭》的规则文本，其组织结构与其中的特殊标记语法参见 [README.md](references/README.md)。

在YAML文件中，所有键为 `text`、`snippet` 和 `toc_entry` 的值都是需要翻译的文本内容。你需要将这些内容翻译为中文，并严格保持其他内容不变，完成后仍以YAML格式保存至 `translation/output`，文件名与原YAML文件相同（如果 `translation/output` 已存在该文件，将其覆盖）。在翻译的过程中，你必须严格遵循 **翻译规范** 中规定的原则。
...
* 由于翻译的内容是游戏的规则，因此必须严格保证译文的含义准确（指含义与英文原文所表达的意思完全一致）和语句通顺。在一些极端情况下，如果这两者无法同时保证，优先保证译文含义的准确性，并输出原文内容和译文内容，同时报告无法同时保证的原因。
...
* 如果 `translation/output/` 中有已存在的翻译后的文档，忽略之，在翻译过程中不要参考这些已有的内容。
* 如果在翻译的过程中，出现了你认为需要理解的概念或术语，可以浏览 `translation/input/` 中的其他规则文档。
* 除了 `translation/input/` 中的文档原文和 [reference/](references/) 文件夹中的参考资料，项目中的其他文件与本次翻译任务无关，你无需查看这些无关文件。
...
在翻译结束后，全文浏览你的译文，对译文内容进行审核与验收。审核和验收的标准是判断译文是否符合如下的评价指标：

1. 检查YAML文件格式是否正确
2. 检查本次任务在 `translation/output` 生成的YAML文档，与原先位于 `translation/input` 中的原文的YAML文档，除了被翻译的部分，其他内容是否严格保持了一致
3. 判断译文与原文的含义是否严格一致
4. 判断译文是否严格遵循了 [specification.md](references/specification.md) 中对翻译的各项要求
5. 判断译文中的用词是否遵循 [terminology.json](references/terminology.json)、[phrasing.json](references/phrasing.json)、[wording.json](references/wording.json) 等术语表中的定义
6. 判断译文是否通顺，是否有语法错误，是否符合中文的语言习惯
7. 判断译文中的用词、用语、句式等风格是否与全文中的其他内容保持了一致，全文的文字风格是否保持统一

如果译文中出现了上述问题，你要对这些问题进行修改，在修改完成后再次进行 **审核验收**，直到译文完全符合上述的评价标准。
